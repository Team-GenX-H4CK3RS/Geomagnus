...
# Step 2: Get the current weather data using OpenWeatherMap API
...
# Step 3: Get the 5-day rainfall forecast using OpenWeatherMap API
def get_rainfall_forecast(latitude, longitude, openweather_api_key):
    ...
    if response.status_code == 200:
        ...
        for entry in forecast_data:
            # Check if rainfall data is available for the time period
            rain_data = entry.get('rain', {}).get('3h', 0)  # Rainfall over the last 3 hours, default to 0 if no data
...
# Step 4: Get the soil moisture data using Open-Meteo API
